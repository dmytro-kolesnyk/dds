...
type DdsMsgServer struct {
	S *Server
	C *Client
}

func (rcv *DdsMsgServer) StartServer(port string) error {
	rcv.S = NewServer()
...
func (rcv *DdsMsgServer) StartClient(addr string, port string) error {
	rcv.C = &Client{}
...
func main() {
	...
	ddsMsgServer := &DdsMsgServer{}
	if len(connect) > 0 {
		if err := ddsMsgServer.StartClient(connect, Port); err != nil {
			log.Println(err)
		}
	...
	if err := ddsMsgServer.StartServer(Port); err != nil {
		log.Println(err)
	}
